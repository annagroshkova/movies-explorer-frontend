...
export default function Navbar() {
  ...
  const navLinkClassName = ({ isActive }) => `navbar__link ${isActive ? 'navbar__link_active' : ''}`
  ...
  return (
    <div className="navbar">
      ...
      <div className={`navbar__menu-container ${open ? "navbar__menu-container_active" : ""}`}>
        { open && <button className="navbar__close-btn" onClick={toggleOpen}><img className="navbar__close-icon" src={closeIcon} alt="Иконка крестик"/></button>}
